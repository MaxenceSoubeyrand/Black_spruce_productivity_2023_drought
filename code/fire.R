...
###
fire <- st_read("C:/Users/msoubeyr/OneDrive - NRCan RNCan/Documents/NPP_drought/github/feux2023/nbac_2023_20240530.shp") %>% 
  select(geometry) %>% 
...
#Ok alors il faut faire deux zooms, un à l'est et l'autre à l'ouest
bc_al <- ggplot() +
  geom_sf(data=canada) +
  geom_sf(data=US) +
  geom_sf(data=NPP_grid, aes(fill=NPP_sd, color=NPP_sd))+
...
#Extraire les points dans le feu
...
NPP_fire <- NPP_grid %>% 
  mutate(fire=clim_id %in% clim_id_fire$clim_id,
         fire=ifelse(fire,"Fire", "No fire"))

ggplot(NPP_fire, aes(y=NPP_sd, x=fire)) +
  geom_boxplot()

ggplot()+
  geom_sf(data=NPP_fire, aes(fill=NPP_sd, color=NPP_sd))

ggplot()+
  geom_sf(data=clim_id_fire, fill="black", color="black")


# Intersection entre la grille et les feux
intersect_fire <- st_intersection(filter(data_points, clim_id%in%clim_id_fire$clim_id), fire)

# Calcul de l'aire brûlée dans chaque cellule
...
ggplot()+
  geom_sf(data=canada)+
  geom_sf(data=NPP_fire, aes(geometry = geometry), fill="black", color="black")
...
fire_diff_plot <- ggplot(NPP_fire2, aes(y = NPP_sd, x = fire)) +
  ...
  stat_compare_means(
    method = "wilcox.test", # Ou t.test si non paramétrique
    label.x = 1.25, 
    label.y = 3.1,
    size = 5
  ) +
...
png(filename = "C:/Users/msoubeyr/OneDrive - NRCan RNCan/Documents/NPP_drought/github/figures/fire_maps.png", 
    width=10, height=8.3, units = "in", res=500)
maps_boxplot
dev.off()

pdf(file = "C:/Users/msoubeyr/OneDrive - NRCan RNCan/Documents/NPP_drought/github/figures/fire_maps.pdf", 
    width=10, height=8.3)
maps_boxplot
dev.off()
